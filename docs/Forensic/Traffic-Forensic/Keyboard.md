...
首先对流量包内的数据进行处理，使用 `usbhid.data` 作为筛选器进行数据筛选，将 hid 数据流提取出来

```shell
tshark -r keyboard.pcapng -T fields -e usbhid.data | sed '/^\s*$/d' > usbdata.txt
```

然后使用 Python 脚本对 hid 数据进行处理，通过字典映射将 hid 数据转换为键盘输入的实际数据
...
用户输入的原始行为就被转化并储存在 output 变量中, 进一步分析大写键和删除键的行为
